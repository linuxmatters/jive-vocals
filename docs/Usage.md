...
In-depth guide to driving Jive Vocals and reading what it produces. For the basic flag table and first-run examples, see the README. For the pipeline itself, see [Pipeline.md](Pipeline.md).
...
**Recording** grades your source capture, the raw audio you fed in. This is the one that varies, and the one you can act on. **Processed** grades the output against the -16 LUFS broadcast target, and it is usually five stars, because hitting that target is Jive Vocals' job and it reliably does. Side by side, the pair tells the story: we took your two-star capture to a five-star master.
...
A low Recording star is a hint to improve the capture next time: record in a quieter room, back the gain off so peaks do not clip, and get the level up if it is too quiet. Either way, Jive Vocals still rescues the file to a broadcast-ready master.

## Analysis-Only Mode

Pass `--analysis-only` to run only Pass 1 analysis. It writes a Markdown analysis report (`<input>-analysis.md`) next to each input and shows the Recording stars plus gain advice on screen, without producing any processed audio. Useful for quickly understanding what Jive Vocals sees in your recordings, diagnosing setup problems, or checking whether a file needs processing at all.
...
- **Filter adaptation**: the exact parameters Jive Vocals would apply, including highpass frequency, gate threshold, NR settings, de-esser intensity, and levelling-compressor configuration
...
For scripts and batch runs: jive-vocals exits 0 on success and 1 when any file fails to process or analyse. Quitting early with `q` or `Ctrl+C` exits 0. Failures writing the report, sidecars, or spectrograms print warnings but leave the exit code untouched.
